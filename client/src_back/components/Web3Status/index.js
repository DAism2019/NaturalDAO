...
export default function Web3Status() {
  ...
  const { active, error,account, connectorName, setConnector } = useWeb3Context()
  console.log(active, account, connectorName, setConnector,error)
  ...
  async function onClick() {
       try {
           ...
           let result = await context.setFirstValidConnector(['Injected', 'Network']);
           console.log(result)
       }catch(err){
           console.log(err)
       }
