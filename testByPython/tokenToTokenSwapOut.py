...
def test():
    ...
    if isTransfer:
        unicorn_txn = myContract.functions.tokenToTokenTransferOutput(5 * 10**3, 10 ** 12, 10 ** 12, deadline,transfer_address,token2_adddress).buildTransaction({
            'nonce': nonce,
            'gas': 400000
        })
    else:
        unicorn_txn = myContract.functions.tokenToTokenSwapOutput(5 * 10**3, 10 ** 12, 10 ** 12, deadline, token2_adddress).buildTransaction({
            'nonce': nonce,
            'gas': 400000
        })
