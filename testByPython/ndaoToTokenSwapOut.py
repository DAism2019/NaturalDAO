...
def buy():
    ...
    if isTransfer:
        unicorn_txn = myContract.functions.ndaoToTokenTransferOutput( 5 * 10**3, 10**10, deadline,transfer_address).buildTransaction({
            'nonce': nonce,
            'gas': 400000
        })
    else:
        unicorn_txn = myContract.functions.ndaoToTokenSwapOutput( 5 * 10**3, 10**10, deadline).buildTransaction({
            'nonce': nonce,
            'gas': 400000
        })
