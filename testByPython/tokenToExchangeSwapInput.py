...
def test():
    ...
    if isTransfer:
        unicorn_txn = myContract.functions.tokenToExchangeTransferInput(5 * 10**3, 1, 1, deadline,transfer_address,exchange2_address).buildTransaction({
            'nonce': nonce,
            'gas': 400000
        })
    else:
        unicorn_txn = myContract.functions.tokenToExchangeSwapInput(5 * 10**3, 1, 1, deadline, exchange2_address).buildTransaction({
            'nonce': nonce,
            'gas': 400000
        })
